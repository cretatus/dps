var platformController = angular.module('cometa');

platformController.controller('PlatformController', function ($scope, $http, ngDialog, $q) {
	...
	$scope.save = function(){
		$http.post('/save/platform', $scope.platform)
			.success(function(data, status, headers, config){
	...
	$scope.remove = function(platform){
        ...
        ngDialog.openConfirm({
        	...
        	function () {
        		$http.post('/remove/platform', platform)
    			.success(function (data, status, headers, config) {
    				$scope.popupMessage = 'Done';
    				ngDialog.open({template: 'popup', scope: $scope});
    				$scope.show();
